...
def read_pickle_files(path: str = "results", num_features: int = 24):
    ...
    columns_features += [f"feature {i}" for i in range(num_features)]
    ...
    oracle_values_dict = (
        ...
        if oracle_dir.exists()
            ...
            dst_train: {
                ...
                for dst_test in datasets
            }
            for dst_train in datasets
        }
    )
...
############################################################################################################
def calculate_fi(data_path: str, path_results: str = "results", num_features: int = 180):
    
    print("Starting the execution of the function")
    print(f'Path to the results: {path_results}')
    ...
    (
        ...
        df_features_dt,
        df_trustworthines,
    ) = read_pickle_files(path_results, num_features=num_features)

    # Calculate the feature importance for each dataset, reduce on and classifier
    for classifier, reduce, dst_train in tqdm(
        list(product(classifiers, reduce_on, datasets))
    ):
        for dst_test in datasets:
            train, test = load_dataset(
                dst_train, dst_test, reduce, path=Path(data_path)
            )
            ...
            for xai in ["oracle", "shap", "lime"]:
                ...
                if doc[dst_train][dst_test][reduce][classifier] is not None:
                    ...
                    if classifier == "Random Forest":
                        ...
                        if (
                            df_features_rf[
                                (df_features_rf["Train"] == dst_train)
                                & (df_features_rf["Test"] == dst_test)
                    ...
                    if classifier == "Decision Tree":
                        ...
                        if (
                            df_features_dt[
                                (df_features_dt["Train"] == dst_train)
                                & (df_features_dt["Test"] == dst_test)
            ...
            if not flag_oracle:
                ...
                #     dst_train=, reduce, latent_dim=latent_dim, data_path=data_path
                # )
                oracle_values_dict[dst_train][dst_test][reduce][classifier] = oracle_values
            ...
            if not flag_shap:
                ...
                shap_values_dict[dst_train][dst_test][reduce][classifier] = shap_values
            ...
            if not flag_lime:
                ...
                lime_values_dict[dst_train][dst_test][reduce][classifier] = lime_values
            ...
            if classifier == "Random Forest" and not flag_rf:
                ...
                df = pd.DataFrame(
                    [[dst_train, dst_test, reduce, classifier] + list(importances)],
            ...
            for i, sample in enumerate(test.X):
                ...
                if (
                    df_trustworthines[
                        (df_trustworthines["Train"] == dst_train)
                        & (df_trustworthines["Test"] == dst_test)
...
if __name__ == "__main__":
    ...
    # Calculate the shap values and lime values for each dataset, reduce on and classifier
    calculate_fi(data_path, path_results)
